...
def lead_api(camp_id,lead):
        conn = http.client.HTTPConnection("crm.nivabupa.com")
...
def api_hit():
        conn = http.client.HTTPSConnection("dial.nivabupa.com")
        payload = ''
        headers = {}
					...
					if int(pace) >= 1:
						...
						if result1:
							...
							if result2:
								...
								agent_count1 = math.ceil(agent_count/2)
								leads_str = lead_api(camp_id,agent_count1)
								...
								if leads:
									for dial_lead in leads:
										...
										if isinstance(conn_5038,telnetlib.Telnet):
											log.info(f"CONNECTION BREAK.")
											conn_5038 = tel(5038, auth=True)
										log.info(f"{cmd}")
							...
							log.info(f"{camp_id}: Agents Not Created. .")
					else:
						log.info(f"{camp_id}: PACING is Zero. Max Call Abdn.")
					time.sleep(0.1)
			else:
				log.info(f"OFFCALL: DIALER STOP DUE TO OFF CALL.")
...
fnc  = {
    'prd_dial': [PRD_DIAL, (1,)],
    'pace_mon': [PACE_MON, (2,)],
    'hang_pack': [END_EVENT, (3,)]
    #'prg_dial': [PROG_DIAL, (4,)]
}
